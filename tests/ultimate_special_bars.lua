...
check(U.bars.primary.abilityId==101 and U.bars.backup.abilityId==102,"Special-bar display does not overwrite either native weapon ability")
...
check(U:GetHUDBar("primary").effectRemaining==0 and U:GetHUDBar("primary").name=="" and U:GetHUDBar("primary").icon=="", "Invalid action types clear stale effect and presentation metadata")
slots[4].actionType=ACTION_TYPE_ABILITY;slots[4].cost=0;slots[4].remaining=0;U:Refresh("unknown cost")
check(U:GetHUDBar("primary").state=="unknown" and not U:GetHUDBar("primary").ready,"An unknown or zero cost never implies charging or readiness")
