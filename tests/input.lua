...
combat = true; events[EVENT_PLAYER_COMBAT_STATE](nil, true)
events[EVENT_GAMEPAD_PREFERRED_MODE_CHANGED]()
scene = {GetName = function() return "inventory" end}
callbacks.SceneStateChanged(scene, nil, SCENE_SHOWING)
check(window:IsHidden() and not Input.suspendedWindow, "An external scene dismisses settings suspended by combat even after an input-mode change")
combat = false; events[EVENT_PLAYER_COMBAT_STATE](nil, false)
check(not listener and not next(layers), "Ending combat cannot reclaim input over the newly opened native scene")
scene = base; window:SetHidden(false)
events[EVENT_PLAYER_DEACTIVATED]()
scene = {GetName = function() return "inventory" end}
callbacks.SceneStateChanged(scene, nil, SCENE_SHOWING)
events[EVENT_PLAYER_ACTIVATED]()
check(window:IsHidden() and not listener and not Input.suspendedWindow, "Loading cannot resurrect a window dismissed by a scene transition")
scene = base; window:SetHidden(false)
combat = true; events[EVENT_PLAYER_COMBAT_STATE](nil, true)
scene = {GetName = function() return "hudui" end}
callbacks.SceneStateChanged(scene, nil, SCENE_SHOWING)
combat = false; events[EVENT_PLAYER_COMBAT_STATE](nil, false)
check(not window:IsHidden() and listener == Input, "A HUD cursor-mode transition preserves the suspended settings context")
