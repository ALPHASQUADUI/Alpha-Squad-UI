...
Candidate: **2.7.0-support-coverage-test.5** / **20705** on **support-coverage**. This checklist is not a record of completed ESO tests.
...
Also validate every packaged Lua file, manifest paths/order/metadata, Core/manifest version consistency and both full-suite/companion package contents. Use current test output for assertion counts; older candidates' counts are not acceptance results for this version.

## In-game acceptance

Download the candidate from the [branch build instructions](../releases/README.md). Record the commit, ESO API version, addon/library versions and relevant screenshots with each finding. Close ESO and back up existing addon folders and SavedVariables before installation. Start with sharing OFF.
...
| Information/provider hovers | Readable effect/source names, conditions and duplicate providers; no raw-ID clutter |
...
| Build inspector | Every selected account shows its own supported snapshot; equipment order, jewelry/weapons, glyphs and missing slots are clear |
...
| Layout | At 720p, 1080p and ultrawide, lists scroll; content does not escape windows; tooltips remain readable; opening a related panel does not create obstructive overlap |
