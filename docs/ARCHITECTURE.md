...
Personal tracking never mutates native Ultimate controls. The former presentation bridge and its style hooks are removed: no native alpha, visibility, mouse input or handler is changed in any addon lifecycle state. ESO and other action-bar addons retain ownership. The larger active-bar marker uses a single native arrow texture per card instead of nine scanline textures. An already stopped progress animation performs no geometry writes during hidden refreshes.

`ULTGroupCatalog.lua` defines nineteen versioned support families, with seven defaults and explicit morph identities. Native progression data may resolve a rank variant to its known morph; class, names, equipped sets and effect IDs cannot invent a slotted Ultimate. Cryptcanon requires the actual shared dedicated ability ID `195031` and reported cost. The food marker reads matching fresh Support Coverage evidence and preserves active/absent/unknown as distinct states; it adds no scanner or network stream. Detail decoding preserves the originating snapshot time, and local known buff expiry is not refreshed merely by rereading the same buff.

`ULTGroupSync.lua` owns provisional protocol **511 / AlphaSquadRaidSettings**, separate from build protocols. Native current membership and crown determine authority: only the crown assigns a raid leader, and only the designated account edits shared family filters. The wire carries bounded settings metadata, catalog version and context/revision data rather than names or builds. Group changes invalidate authority; stale, unauthorized and mismatched-context updates cannot replace accepted state. Native protocol OFF blocks both send and receive, with local filters used while unavailable. Missing synchronization is surfaced rather than represented as confirmed agreement.

Its independent handler is `AlphaSquadUIRaidSettings` / `ASUIRaidSettings`. A delegated leader proposes filters to the crown, which relays the accepted state. Numeric payload fields total 166 bits before the library header; they contain no account names, items or build history. Remote coordination stays transient and solo filters remain local. Regular sends are coalesced with a 1.5-second minimum interval and at most two repeats of the latest state/proposal. Initial synchronization uses at most three requests separated by 30 seconds; opening settings can request recovery at most once per ten seconds. Incoming peer requests are rate-limited. Combat/loading prevents settings edits and sends; replacement of an already queued owned frame is best effort because the library exposes no public cancellation API.
...
MOVE HUD hides configuration windows before placement. Done/Back saves and locks placement, then selects Alpha Squad in native Settings. Safety termination clears the request and return target without displaying settings. Input ownership follows the visible destination and releases during native dialog or non-gameplay takeover.
...
Addon-owned English source keys resolve through a French dictionary when selected. English/French are the only preferences; a supported client locale seeds the saved choice once, otherwise English. Legacy automatic preferences migrate to an explicit choice. Static controls are bound once; dynamic renderers refresh cached presentation without scanning equipment, sending builds or changing sharing consent.

`Localization/NativeNames.lua` supplies bounded, ID-based English/French mappings for verified supported names. Unmapped IDs preserve native fallback text without inferring identity from a translated name. Protocol tokens, saved keys, item links and player names remain untouched. Complete native tooltips still use the client language. Text controls constrain wrapping/ellipsis and layouts reflow; real-client checks remain required for font metrics and UI scales.
