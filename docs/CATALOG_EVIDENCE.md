...
| [Official Update 49 notes](https://forums.elderscrollsonline.com/en/discussion/689473/update-49-live-patch-notes-all-platforms) | Published 9 March 2026; revisited 15 September | Warmth renamed Traumatic Burns; The Storm Voice occupies the former Scaled Armor position in Draconic Power, while the former Battle Roar position is Landslide |
| [Native The Storm Voice tooltip and image](https://eso-hub.com/en/skills/dragonknight/draconic-power/the-storm-voice) | Game-data export inspected 15 September 2026 | Native texture `ability_dragonknight_031_u49_new`; the native API resolves the reported passive ID/rank at runtime, avoiding invented static IDs |
...
The native Scribing definition APIs, called with the reported grimoire and script IDs, establish script positions and legal recipe membership. The native texture identities for the twelve grimoires and scripts are independently present in the [maintainer's native texture inventory](https://github.com/DakJaniels/LuiExtended/blob/27c16a1ce0ffb64beb66274c5730853d197ef996/LuiData/PC/IconFrameColorSamples.lua). No guessed numeric Scribing IDs are introduced. The implementation uses the game's per-grimoire script tooltips rather than a generic interpretation of script names; unsupported recipes remain unknown.

### Scribing tooltip evidence

The linked pages expose game-generated descriptions separately for each grimoire. Reviewed 15 September 2026. [The catalog recipient matrix](SUPPORT_COVERAGE_CATALOG.md#scribing-recipient-rules) records the implemented subset; healing amounts and other viewer-scaled values are not copied into the availability rules.

| Native script descriptions | Evidence used |
|---|---|
| [Healing](https://eso-hub.com/en/scribing/scripts/healing), [Damage Shield](https://eso-hub.com/en/scribing/scripts/damage-shield), [Restore Resources](https://eso-hub.com/en/scribing/scripts/restore-resources) | Which Focus targets allies and makes beneficial Affixes apply to them; Mender's link includes linked allies independently of damage Focus rules |
| [Breach](https://eso-hub.com/en/scribing/scripts/breach), [Cowardice](https://eso-hub.com/en/scribing/scripts/cowardice), [Maim](https://eso-hub.com/en/scribing/scripts/maim) | Exact per-grimoire Major/Minor distinction and enemy recipients |
| [Vulnerability](https://eso-hub.com/en/scribing/scripts/vulnerability), [Brittle](https://eso-hub.com/en/scribing/scripts/brittle), [Off Balance](https://eso-hub.com/en/scribing/scripts/off-balance) | Specific enemy-affecting grimoires, including enemies crossing a Mender's link |
| [Lifesteal](https://eso-hub.com/en/scribing/scripts/lifesteal), [Magickasteal](https://eso-hub.com/en/scribing/scripts/magickasteal) | Minor recovery debuffs and the grimoires on which they apply |
| [Heroism](https://eso-hub.com/en/scribing/scripts/heroism), [Protection](https://eso-hub.com/en/scribing/scripts/protection) | Mender and supportive Torchbearer recipients differ from caster-only Trample bonuses |
| [Berserk](https://eso-hub.com/en/scribing/scripts/berserk), [Force](https://eso-hub.com/en/scribing/scripts/force) | Healing/shield recipes can support allies; Traveling Knife's bonuses explicitly target its caster |
| [Courage](https://eso-hub.com/en/scribing/scripts/courage), [Resolve](https://eso-hub.com/en/scribing/scripts/resolve), [Vitality](https://eso-hub.com/en/scribing/scripts/vitality), [Intellect and Endurance](https://eso-hub.com/en/scribing/scripts/intellect-and-endurance), [Empower](https://eso-hub.com/en/scribing/scripts/empower) | Each grimoire's exact tier, with an allied Focus/link required before assigning an ally source |
| [Warrior's Opportunity](https://eso-hub.com/en/scribing/scripts/warriors-opportunity) | Traveling Knife alone applies the enemy Martial Damage taken bonus; other grimoires have different personal/damage effects. Native Signature texture is `scribing_secondary_opportunism.dds`. |
...
| Learned-name prerequisites disappeared after sharing or on translated clients | Traumatic Burns uses the former Warmth passive IDs `29430 / 45012` from the cited native registry and official rename. The Storm Voice resolves the reported learned ID using `GetSpecificSkillAbilityKeysByAbilityId`, the native Draconic Power line, passive flag, rank and texture. Learned labels are no longer capability evidence. |
| Scribing only recognized Banner | Eleven additional explicit grimoire matrices preserve exact Major/Minor tiers and ally/enemy recipients. Warrior's Opportunity is recognized only on damaging Traveling Knife. |
...
- Native name and slot IDs outrank received labels. Learned passive and selected mastery coverage use IDs only. The Storm Voice additionally requires native passive-definition resolution; missing APIs stay unknown. Combat bundles `259215 / 259216 / 259217`, Landslide and Elder Dragon are not substitutes for the prerequisite. No independent static learned Storm Voice ID is claimed in the catalog.
...
- The twelve Scribing grimoires have finite explicit matrices for existing selectable effects. Enemy debuffs require a recognized enemy-targeting Focus or a Mender link; allied Affixes require a verified allied Focus or link. Other signatures and new/unrecognized combinations remain inspectable without automatic capability. The Class Mastery Scribing script is never treated as selected pure-class mastery.
