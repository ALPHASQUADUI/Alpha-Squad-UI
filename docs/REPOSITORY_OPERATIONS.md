...
The maintainer previously reported 3.3.1 stable. That acceptance does not apply to the 3.4 interface or its 3.4.1 dropdown hotfix; the maintainer reported a native initialization failure in 3.4.0 and the corrected build awaits retesting. The current cycle authorizes dev changes only, with no PR or release. The procedure below applies only after separate approval of the exact version and a verified human merge. Prerequisites requiring external evidence or administration must never be marked complete merely to make the workflow pass.
...
5. Once the evidence and settings are real, set the four prerequisite flags and `publish` to `true` in [release-policy.json](../tooling/release-policy.json). Keep `source_branch: main` and `version: 3.4.1`. Update the release notes to the actual completed status, then have a human review and merge the PR to `main` after successful CI. The assistant cannot perform this step.
6. In **Actions → Validate AlphaSquadUI → Run workflow**, select `main`, enable `publish`, and enter `3.4.1`. Review the exact validated commit and approve the `release` environment when requested. A push or an ordinary validation run does not publish a release.
7. Verify that `v3.4.1` resolves to that run's source commit and that its five release assets are the two installable ZIPs, `RELEASE_NOTES.md`, `release.json` and `SHA256SUMS`. Verify the hashes and the public download links before announcing the release.
