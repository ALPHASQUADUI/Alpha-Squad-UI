...
- Keep code, identifiers, comments, commits and public documentation in English. The interface supports explicit English/French choices; seed the saved Dashboard language once from a supported client locale, otherwise English. Translate addon-owned presentation and verified supported game-name mappings, never protocol keys, native item links or player names. Unresolved game names and full native tooltips retain their native client text. Keep French and English layouts readable and bounded.
...
- Keep HUD placement centralized. The whole selected panel body is draggable, including its rows/icons. Corners scale proportionally, edges reshape/reflow content, and the toolbar owns supported background opacity, scale, fit and reset. Hide irrelevant opacity controls for transparent HUDs. Preserve disabled modules and normal visibility choices; save/lock on completion and stop placement during combat/loading.
- Personal Ultimate uses the selected transparent design 2: native icon, actual points/native cost, thin progress bar and concise localized state, with both normal weapon slots. Do not restore AUTO/FRONT/BACK/BOTH selectors. Show the smooth green left-side marker only for the verified active bar, dim the inactive slot, and preserve unknown costs and special-bar uncertainty. Overload shares this HUD with actual remaining points and a reserve marker; its reminder threshold is not a cost. Use neutral inactive, gold active, green ready pulse and red stop pulse. Never hide, restyle, hook or disable the native Ultimate controls; ESO and other action-bar addons own their presentation and input.
- Group Ultimate filters use verified support families and morph IDs, including Cryptcanon's actual dedicated Ultimate. The crowned group leader alone assigns the raid leader; the designated raid leader alone changes shared filters. Never trust a payload's claimed authority without current native membership/leader checks. Settings-only protocol 511 remains provisional pending formal reservation/coexistence; respect native OFF and never enable build/Ultimate sharing through it. Missing, expired or incomplete food evidence remains Unknown, not confirmed absence.
