...
-- Read-only resource warnings. Ultimate activation remains under player control.
...
function O:Suppressed()
    ...
    if not config or not config.enabled or (ULT.HasSpecialActiveBar and ULT:HasSpecialActiveBar()) then return true end
...
function O:FindActiveBuff()
    ...
    for index=1,count do
        local name,_,_,_,_,icon,_,_,_,_,abilityId=GetUnitBuffInfo("player",index)
        if self:IsAbility(abilityId,name,icon) then return true end
    end
end
function O:PlayAlert(ready,force)
    ...
    local interval=ready and 12000 or 2200
...
function O:Update(current,reason)
    ...
    if not config or not priority then
        self.active=false;self.level="none";self.expectedUntil=nil;self.ready=false
    ...
    self.level="none"
    if self.active and config.reserveAlertsEnabled and current<=config.reserveWarningThreshold then
        self.level="warning"
    end
...
function O:UpdateRuntime()
    ...
    if not active then
        ...
        self.active=false;self.level="none";self.ready=false
        return
    end
...
function O:SetOption(key,value)
    ...
    config[key]=value
    self.lastReadyAt=0;self:UpdateRuntime()
...
function O:Migrate(finalize)
    ...
    -- Preserve an explicit warning preference while retiring cancellation settings.
    if sv.overload.reserveAlertsEnabled==nil and type(sv.overload.reserveCutoffEnabled)=="boolean" then
        sv.overload.reserveAlertsEnabled=sv.overload.reserveCutoffEnabled
    end
    sv.overload.reserveCutoffEnabled=nil;sv.overload.reserveThreshold=nil
    if sv.unifiedUltimateVersion~=1 then
        ...
        for key,default in pairs(defaults) do
            ...
            if key=="reserveAlertsEnabled" then source=legacy.reserveCutoffEnabled end
...
function O:RegisterCommands()
    SLASH_COMMANDS["/asoverload"]=function(argument)
        ...
        elseif command=="reserve on" or command=="reserve off" then O:SetOption("reserveAlertsEnabled",command=="reserve on")
        elseif command=="reserve" or command=="reserve toggle" then O:SetOption("reserveAlertsEnabled",not O:Config().reserveAlertsEnabled)
        ...
        else if d then d("/asoverload • settings • on/off • /asmove") end end
    end
end
