...
[Release downloads](https://github.com/ALPHASQUADUI/Alpha-Squad-UI/releases) · [Build artifacts](https://github.com/ALPHASQUADUI/Alpha-Squad-UI/actions) · [Version 3.3.0 notes](3.3.0.md)

## Install

1. Close ESO or return to character selection. Back up existing addon SavedVariables before updating.
2. Extract **AlphaSquadUI-3.3.0.zip** so `AlphaSquadUI/AlphaSquadUI.txt` is directly inside the ESO `live/AddOns` directory.
...
The optional **AlphaSquadBuildShare-3.3.0.zip** installs a separate `AlphaSquadBuildShare` directory for players who want to share supported builds without the tracking interface. It requires LibGroupBroadcast and its dependencies, starts sharing on a new installation and preserves an existing OFF preference. Use `/asbuildshare off`, `/asbuildshare on` and `/asbuildshare status` to control it. Install the appropriate package; the full suite already includes build sharing. The companion transmits build snapshots, not live Ultimate charge; that needs a compatible LibGroupCombatStats sender.

## Updating to 3.3.0

Update the full suite and any separately installed companion together. Settings reflow on narrow screens, Group Ultimate rows retain their configured height as the roster changes, and the simplified Move HUD toolbar keeps positioning controls together. Unchanged build refreshes preserve open tooltips; duplicate contributors can be selected directly. Native sharing status and same-session set restoration are corrected.
