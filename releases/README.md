...
[Release downloads](https://github.com/ALPHASQUADUI/Alpha-Squad-UI/releases) · [Build artifacts](https://github.com/ALPHASQUADUI/Alpha-Squad-UI/actions) · [Version 3.6.0 notes](3.6.0.md)

The current development package is **3.6.0**, with corrected HUD dragging, explicit EN/FR selection, verified support Ultimate families, food uncertainty and raid-leader filter coordination. Personal tracking leaves ESO's native Ultimate button untouched. Download it from a successful `dev` Actions run and verify the version/source commit. Native acceptance remains pending; this cycle authorizes no PR or public release.

The maintainer's 3.3.1 stability report is historical and does not approve 3.6.0. Public release requires separate authorization, formal protocol reservation, coexistence evidence and release-environment configuration. Retain a private backup of your existing SavedVariables when installing the candidate.

## Install

1. Close ESO or return to character selection. Back up existing addon SavedVariables before updating.
2. Extract **AlphaSquadUI-3.6.0.zip** so `AlphaSquadUI/AlphaSquadUI.txt` is directly inside the ESO `live/AddOns` directory.
...
The optional **AlphaSquadBuildShare-3.6.0.zip** installs a separate `AlphaSquadBuildShare` directory for players who want to share supported builds without the tracking interface. It requires LibGroupBroadcast and its dependencies, starts with sharing OFF on a new installation and preserves existing saved preferences. Use `/asbuildshare off`, `/asbuildshare on` and `/asbuildshare status` to control it. The full suite already includes build sharing. The companion transmits build snapshots, not live Ultimate charge or raid settings; live charge needs a compatible LibGroupCombatStats sender, and coordinated filters require the full suite.

## Updating to 3.6.0

Update the full suite and any separately installed companion together, then log in or `/reloadui`. This reload removes the retired 3.5.0 native-slot hooks; Alpha Squad no longer changes that button in any tracking state. Alternate the Dashboard theme/language menus and verify independent selection without duplicate-control errors. Only English and French are offered; a supported client language seeds the saved choice once. Check both personal orientations, Overload states, group-panel body dragging and the raid-leader selector. Record exact native outcomes in [client acceptance](../docs/CLIENT_ACCEPTANCE.md).

The crowned group leader assigns a raid leader; that player's curated family toggles are authoritative for compatible clients. Settings-only protocol 511 is separate from build/Ultimate sharing and respects native OFF. When synchronization is unavailable, status must say so. Group food markers reuse fresh known evidence and show unknown for missing or expired reports. The [current acceptance route](../docs/SUPPORT_COVERAGE_TESTING.md#360-interface-and-raid-coordination-acceptance) covers multiplayer and language boundaries.
...
Existing supported settings and placement remain intact. The personal Ultimate HUD displays both normal weapon slots with native icons, actual points/native costs, thin progress bars, a green active-bar marker and a dimmed inactive slot; the old AUTO/FRONT/BACK/BOTH selectors are retired. Overload remains integrated into this transparent HUD with actual remaining points, a reserve marker and neutral inactive, gold active, green ready and red stop states, plus **Warning starts** and optional alerts under **ULT Tracker → OVERLOAD SETTINGS**. Its reminder threshold is not a native skill cost. Automatic cancellation has been removed; you stop Overload using your Ultimate binding. Group tracking has one ON/OFF switch in the parent ULT settings and excludes your own character. A previously hidden group HUD migrates to Group tracking OFF, while filters and layout remain saved.
